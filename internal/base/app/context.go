...
type Context struct {
	...
	ssoID    string
	log      string // For ULMS, log message
	ip       string
...
func (ctx *Context) GetLog() string       { return ctx.log }
func (ctx *Context) SetLog(log string)    { ctx.log = log }
func (ctx *Context) AppendLog(log string) { ctx.log = ctx.log + log }
...
func NewContext(rw http.ResponseWriter, r *http.Request, username string, isStaging bool) *Context {
...
func (ctx *Context) GetRequestInfo() string {
	...
	return fmt.Sprintf("%s %s SSO_ID: %s", ctx.Request.Method, ctx.Request.URL.String(), ctx.GetSsoID())
}
